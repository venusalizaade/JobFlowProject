...
public class JobPostController : Controller
{
    ...
    public async Task<IActionResult> MyJobs()
    {
        ...
        ViewBag.LatestPayments = await _dbContext.Payments
            .AsNoTracking()
            .Where(p => jobIds.Contains(p.JobPostId) && !p.IsDeleted)
            .GroupBy(p => p.JobPostId)
            .Select(g => new
            {
                JobPostId = g.Key,
                Status = g.OrderByDescending(p => p.CreatedAt).Select(p => p.Status).FirstOrDefault()
            })
            .ToDictionaryAsync(x => x.JobPostId, x => x.Status);
